...
namespace Poco
{
	namespace Net
	{
		...
		{
		public:
			MessageNotification(const char *buffer, std::size_t length, const Poco::Net::SocketAddress &sourceAddress) : _message(buffer, length),
																														 _sourceAddress(sourceAddress)
			{
			}

			MessageNotification(const std::string &message, const Poco::Net::SocketAddress &sourceAddress) : _message(message),
																											 _sourceAddress(sourceAddress)
			{
			}
		...
		RemoteUDPListener::RemoteUDPListener(Poco::NotificationQueue &queue, Poco::UInt16 port, bool reusePort, int buffer, Poco::Net::IPAddress::Family family) : _queue(queue),
		...
		SyslogParser::SyslogParser(Poco::NotificationQueue &queue, RemoteSyslogListener *pListener) : _queue(queue),
																									  _stopped(false),
		...
		std::string SyslogParser::parseStructuredData(const std::string &line, std::size_t &pos)
		{
			...
			if (pos < line.size())
			{
				...
				if (pos < line.size() && Poco::Ascii::isSpace(line[pos]))
					++pos;
			}
			return sd;
		}
		...
		RemoteSyslogListener::RemoteSyslogListener() : _pListener(0),
													   _pParser(0),
		...
		RemoteSyslogListener::RemoteSyslogListener(Poco::UInt16 port) : _pListener(0),
																		_pParser(0),
																		_port(port),
																		_reusePort(false),
																		_threads(1),
		...
		RemoteSyslogListener::RemoteSyslogListener(Poco::UInt16 port, bool ipv6) : _pListener(0),
																				   _pParser(0),
																				   _port(port),
																				   _reusePort(false),
																				   _threads(1),
		...
		RemoteSyslogListener::RemoteSyslogListener(Poco::UInt16 port, int threads) : _pListener(0),
																					 _pParser(0),
																					 _port(port),
																					 _reusePort(false),
																					 _threads(threads),
		...
		RemoteSyslogListener::RemoteSyslogListener(Poco::UInt16 port, bool reusePort, int threads) : _pListener(0),
																									 _pParser(0),
																									 _port(port),
